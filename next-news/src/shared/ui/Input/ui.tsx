  ...
  return (
    <div className={`flex flex-col w-full md:w-64  ${className ? className : ''}`}>
      ...
      <div className='relative flex items-center overflow-hidden'>
        ...
        <span onClick={() => { setValue(""); if (onClear) onClear(); }} className={`absolute transition-all z-10 cursor-pointer ${icon ? iconPosition === "right" ? 'right-12' : 'right-5' : 'right-5'} ${value == "" && allowClear && !disabled ? '-right-5' : ''}`}><Close color="#6D757F" /></span>
      </div>
    </div>
  );
};
