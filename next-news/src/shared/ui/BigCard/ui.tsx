...
export const UI: React.FC<Props> = ({
  ...
  return (
    <div
      className={`${
        styles.bigCard
      } big-card after:content-[''] after:absolute after:w-full after:h-full after:top-0 after:left-0 after:bg-linear-gradient p-5  relative overflow-hidden rounded-md w-full min-h-64 lg:min-h-96 h-full inline-flex flex-col md:items-start justify-end ${
        className ? className : ""
      }`}
    >
      <div className="absolute w-full h-full inset-0 rounded-md">
        <div
          ...
          className={`${styles.bigCardImg} w-full h-full shadow-2xl transition-all duration-700 scale-100 bg-center bg-cover inset-0`}
        />
      </div>
